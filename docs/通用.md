...
const flat = (arr, size) => {
  if (size > 0) {
    return arr.reduce(
      (pre, cur) => pre.concat(Array.isArray(cur) ? flat(cur, size - 1) : cur),
      []
    )
  } else {
    return arr.slice()
...
- #### SameSite
  ...
  _Chrome80 版本中默认屏蔽了第三方的 Cookie_
...
NodeJs 默认是单线程还是多线程，单线程和多线程的适用场景?
...
2. 为什么需要它呢？
   ...
   1. 由于 vue 的异步更新策略导致我们对数据的修改不会立刻体现在 dom 变化上。此时如果想要立即获取更新后的 dom 状态，就需要使用这个方法。

3. 我再什么地方用它呢？

   1. nextTick 方法会在队列中加入一个回调函数，确保该函数在前面的 dom 操作完成后才调用。
...
┌─────────────┐
│ 宏任务开始 │ <--- 比如执行 setTimeout 回调、click 事件处理器
├─────────────┤
│ 执行同步 JS │
├─────────────┤
│ 执行微任务 │ <--- 所有 promise.then()、queueMicrotask 都在这一步统一清空
├─────────────┤
│ requestAnimationFrame 回调 │ （如果有）
├─────────────┤
│ 浏览器渲染（UI 更新） │
└─────────────┘
↓ 进入下一轮事件循环
...
| 内容                | 属于哪类任务？ | 说明                         |
| ------------------- | -------------- | ---------------------------- |
| 主 JS 脚本执行      | 宏任务         | 最初执行的代码               |
| `setTimeout(fn)`    | 宏任务         | 延迟执行的任务               |
| `click` 等 DOM 事件 | 宏任务         | 事件响应被放入宏任务队列     |
| `Promise.then(fn)`  | 微任务         | 当前宏任务后立即执行         |
| `MutationObserver`  | 微任务         | DOM 变更后执行               |
| 浏览器渲染          | 非任务         | 通常在微任务后、下一帧前发生 |
...
<small>部分 css3 的动画效果是在合成线程上实现的，不需要主线程介入，所以省去了重排和重绘的过程，这就大大提升了渲染效率。JavaScript 都是在在主线程上执行的，所以 JavaScript 的动画需要主线程的参与，所以效率会大打折扣！</small>
...
HTML 缓存、离线包、等，例如可以配置一个预加载列表，在 APP 启动或某些时机时提前去请求，这个预加载列表需要包含所需 H5 模块的页面和资源，还需要考虑到一个 H5 模块有多个页面的情况，这个列表可能会很大，也需要工具生成和管理这个预加载列表。
